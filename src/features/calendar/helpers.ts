...
export function getEventBlockStyle(
  ...
  const eventStart = new Date(Math.max(startDate.getTime(), dayStart.getTime()))
...
export function calculateMonthEventPositions(
  ...
  for (const event of sortedEvents) {
    ...
    const eventDays = eachDayOfInterval({
      start: new Date(Math.max(eventStart.getTime(), monthStart.getTime())),
      end: new Date(Math.min(eventEnd.getTime(), monthEnd.getTime())),
    })
