...
export function rangeText(view: TCalendarView, date: Date): string {
  ...
  switch (view) {
    ...
    case 'day': {
      return format(date, dateFormatPatterns.fullDate)
    }
    case 'agenda': {
      start = startOfMonth(date)
      end = endOfMonth(date)
  ...
  return `${format(start, dateFormatPatterns.fullDate)} - ${format(end, dateFormatPatterns.fullDate)}`
}
...
export function getEventsCount(
  ...
  const compareFns: Record<TCalendarView, (d1: Date, d2: Date) => boolean> = {
    day: isSameDay,
    week: (d1: Date, d2: Date) => isSameWeek(d1, d2, { weekStartsOn: 1 }),
    month: isSameMonth,
    agenda: isSameMonth,
  }
...
export function getEventBlockStyle(
  ...
  const eventStart = Math.max(startDate, dayStart)
...
export function getCalendarCells(selectedDate: Date): ICalendarCell[] {
  ...
  // Adjust for Monday start (Sunday = 0 becomes 6, Monday-Saturday = 1-6 becomes 0-5)
...
export function calculateMonthEventPositions(
  ...
  for (const event of sortedEvents) {
    ...
    const eventDays = eachDayOfInterval({
      start: Math.max(eventStart, monthStart),
      end: Math.min(eventEnd, monthEnd),
    })
...
export function formatTime(
  ...
  return format(parsedDate, use24HourFormat ? "HH'h'mm" : "h'h'mm a")
}
