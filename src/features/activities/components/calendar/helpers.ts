...
export function getEventsCount(
  ...
  return events.filter((event) => compareFn(parseISO(event.startTime), date))
    .length
}
...
export const getEventsForDay = (
  ...
  return events
    .filter((event) => {
      const startOfDayForEventStart = startOfDay(parseISO(event.startTime))
      const startOfDayForEventEnd = startOfDay(parseISO(event.endTime))
      if (isWeek) {
        return (
