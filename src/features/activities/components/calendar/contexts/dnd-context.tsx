...
import { DndConfirmationDialog } from '@/features/activities/components/calendar/dialogs/dnd-confirmation-dialog'
      ...
      try {
        const updatedEvent: EventFormData = {
          ...
          participants: event.participants
            .filter((p) => p.participantType && p.participantId)
            .reduce(
              (
                acc: {
                ...
                const existing = acc.find(
                  (p) => p.participantType === curr.participantType
                )
                if (existing) {
                  existing.participantIds.push(curr.participantId!)
                } else {
                  acc.push({
                    participantType: curr.participantType! as
                      | 'UNIT'
                      | 'TEAM'
                      | 'USER',
                    participantIds: [curr.participantId!],
                  })
                }
                return acc
              },
